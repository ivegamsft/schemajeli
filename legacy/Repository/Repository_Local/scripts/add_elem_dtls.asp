...
strLastMod = CreateTS()
strElementBName = UCASE(Session("strElemBName"))
aryElementBName = SPLIT(strElementBName,"*")
strElementBName= UCASE(JOIN(aryElementBName))
strElementDName = UCASE(Session("strElemDName"))
strElementCName = UCASE(Session("strElemCName"))
aryElementCName = SPLIT(strElementCName,"_")
strElementCName= JOIN(aryElementCName,"_")
strElementKeyWords = UCASE(Session("strKeyWords"))
...
%>  
  <TR>
  <TD ALIGN="Center" COLSPAN="4"><FONT COLOR="<%=strErrTextColor%>"><B>The column name is too long-Select the abbreviations remove
    </BR>
    </FONT>
  </TD>
  </TR>
  <TR>
  <TD COLSPAN="4" ALIGN="center"><INPUT TYPE=HIDDEN NAME=ElementKeyWords VALUE="<%=strElementKeyWords%>">
...
%>  
  ...
  <TD>
   ...
   <OPTION SELECTED>--Select a data type--</OPTION>
      ...
      <OPTION VALUE="TIMESTAMP">TIMSTAMP</OPTION>
...
%>
  ...
  <TD COLSPAN="4">
    <FONT FACE="Verdana" SIZE="2">
    <B>This element was created by <%=strCreator%><BR>
