...
<!--
     <SELECT NAME="Server" SIZE="1">
      <OPTION SELECTED VALUE="<%=strServer%>"><%=strServer%></OPTION>
      <% For i=0 to UBOUND(aryServer) %>
           <OPTION VALUE="<%=aryServer(i)%>"><%=aryServer(i)%></OPTION>
      <% Next %>  
      </SELECT>
...
<TR>
  ...
  <TR>
   <TD><FONT FACE="Verdana" SIZE="2"><B>RDBMS Version</B>
    ...
    <TD>
    <INPUT TYPE="text" NAME="RDBMSVer" VALUE="<%=strRDBMSVer%>" SIZE="8">
    </TD>
  </TR>
...
<TR>
   <TD COLSPAN="4">
     <TEXTAREA ROWS="2" NAME="DbDesc" COLS="80"  NOSOFTBREAKS WRAP="soft"><%=strDbDesc%></TEXTAREA>
   </TD>
</TR>
...
<TR>
   <TD COLSPAN="4">
   <FONT FACE="Verdana" SIZE="2">
     <B>This database was created on <%=strCreateTS%> by <%=strCreateUID%><BR>
